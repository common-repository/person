 ...
 * 
 */
function person_testimonial_render_bootstrap( $post_id, $args = array(), $version = '3' )
{
	// Future proofing, set the version of Bootstrap we wish to render
	// if( $version == '3' ){ // Render here }

	person_testimonial_render_bootstrap_3( $post_id, $args );
}
 ...
 * 
 */
function person_testimonial_render_bootstrap_3( $post_id, $args = array() ) 
{
	$defaults = array(
	...
	$person_related 							= get_post_meta( $post_id , '_person_related', true );

	if( !empty( $person_related ) )
	{
		$person 								= get_post( $person_related );
	}
	else
	{
	...
	{
		$person_name = $testimonial->post_title;
	}
		...
		if( $r['show_content'] )
		{
			...
			<div class="<?php echo $r['class_content_wrapper']; ?>">
				<?php 
					...
					{
						echo wpautop( $testimonial->post_content );
					}
					else if( $r['content_source'] == 'short' )
					{
						$person_testimonial_short = get_post_meta( $testimonial->ID, '_person_testimonial_short', true );
 ...
 * 
 */
function person_testimonial_render_boostrap( $post_id, $args = array(), $version = '3' )
{
	person_testimonial_render_bootstrap( $post_id, $args, $version );
}
?>